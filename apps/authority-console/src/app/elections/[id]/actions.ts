...
import { construirRutaConAviso } from "../../components/ActionNotice";
import {
  canonicalizeJson,
  getPublicKeyHex,
  sha256Hex,
...
export type ElectionPhasesResponse = {
  ok: boolean;
  chainId: string;
...
export function phaseBadgeClass(label: string | undefined): string {
...
export function verificationBadgeClass(status: string | undefined | null): string {
...
export function etiquetaTipoActa(kind: string): string {
...
export function isCatalogMutablePhase(phaseLabel: string): boolean {
...
export async function publishActaAction(formData: FormData) {
  "use server";
  const env = getEnv();
...
export async function createOrUpdateCandidateAction(formData: FormData) {
  "use server";
  const env = getEnv();
...
export function eventLabel(e: ElectoralEventType): string {
...
export async function loadElectionData(electionIdStr: string) {
  const envRes = getEnvResult();
  if (!envRes.ok) return { ok: false as const, envRes };
  ...
  const pool = getPool(env.DATABASE_URL);
  await ensureSchema(pool);
  const adminLog = await listAdminLogEntries({ pool, chainId: env.CHAIN_ID, contractAddress: env.CONTRACT_ADDRESS, electionId, limit: 50 });
  ...
  const evidenceApiUnavailable = !phasesRes || !phaseChangesRes || !anchorsRes || !actsRes || !consistencyRes || !incidentsRes;

  return {
    ok: true as const,
    env, electionIdStr, electionId,
    phasesRes, phaseChangesRes, anchorsRes, actsRes, consistencyRes, incidentsRes,
    adminLog, candidates, currentManifest,
    currentPhaseLabel, catalogMutable,
    hasBallotOrderCollisions, availableEvents,
    activeIncidents, criticalActive, warningActive, infoActive,
    consistencyOk, latestTransition, latestAct,
