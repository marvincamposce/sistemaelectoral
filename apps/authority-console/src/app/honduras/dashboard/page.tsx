    ...
    <div>
      <div className="space-y-8">
        <HondurasHeader
          ...
          recentEligibleCount={recentEligibleRecords}
          pendingEnrollments={pendingEnrollments}
          activeRecentAuthorizations={activeRecentAuthorizations}
        />
          ...
          <div className="space-y-8">
            <div className="admin-card">
              <div className="admin-card-header">
                <h2 className="admin-section-title m-0">Elecciones disponibles</h2>
              </div>
              <div className="admin-card-body p-0">
                {elections.length === 0 ? (
                  <div className="p-6 text-center text-sm text-slate-500">
                    No hay elecciones indexadas todavía.
                  </div>
                ) : (
                  <div className="divide-y divide-slate-100">
                    {elections.map((election) => {
                      ...
                      const phaseKey = String(election.phaseLabel ?? "").toUpperCase();
                      const phaseLabel = PHASE_LABELS_ES[phaseKey] ?? `Fase ${election.phase}`;

                      return (
                        <div key={election.electionId} className="p-4 hover:bg-slate-50 transition-colors">
                          <div className="flex justify-between items-center">
                            <span className="font-semibold text-slate-900">Elección #{election.electionId}</span>
                            <span className="admin-badge admin-badge-info">{phaseLabel}</span>
                          </div>
                          <div className="text-xs text-slate-500 mt-2">
                            {election.counts?.signups ?? 0} inscritos · {election.counts?.ballots ?? 0} votos
                          </div>
                        </div>
                      );
                    })}
