...
const CensusUpsertInputSchema = z
  .object({
    ...
    demoPin: z.string().trim().max(32).optional(),
...
async function upsertCensusRecordAction(formData: FormData) {
  ...
  const parsed = CensusUpsertInputSchema.parse({
    ...
    demoPin: String(formData.get("demoPin") ?? "").trim() || undefined,
  ...
  if (parsed.demoPin) metadataJson.demoPin = parsed.demoPin;
...
export default async function HondurasPage(props: {
  ...
  const [recentRecords, selectedRecord, selectedLinks] = await Promise.all([
          ...
          <div className="space-y-6">
            ...
            <section className="card p-4 space-y-3">
              ...
              <form action={upsertCensusRecordAction} className="grid gap-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <input className="rounded-md border border-slate-300 px-3 py-2 text-sm" name="demoPin" placeholder="PIN demo opcional" />
